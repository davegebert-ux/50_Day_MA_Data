...
THE SIX TRAIL RULES (what get_trail_line() computes each day)
-----------------------------------------------------------------
...
def get_trail_line(df, i, rule, entry_idx, adr_threshold, swing_state):
    ...
    # "no trail line today" on EVERY day -- it silently never trailed and
    # only ever exited on the initial hard stop. A typo in a rule name
    # produced plausible-looking but meaningless results with no error.
    # Fail loudly instead: a misspelled rule is always a bug, never a
    # legitimate request.
    raise ValueError(
...
def simulate_trail(df, entry_idx, entry_price, risk_per_share, rule='atr_1.0x', take_partial=False,
    ...
    stop-out) used to generate the pre-2026-09-03 Outcome_* columns.
      ...
      risk_per_share     : initial risk in dollars/share (defines both the
                           ...
                           Recommended rule = 'atr_1.0x', take_partial = False.
